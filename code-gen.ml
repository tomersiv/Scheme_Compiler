...
| curr :: rest -> (match curr with
                  ...
                  | Sexpr(Number(Fraction(x, y))) -> (create_const_tbl rest (const_table @ [(Sexpr(Number(Fraction(x, y))), (offset, "MAKE_LITERAL_RATIONAL(" ^ (string_of_int x) ^ "," ^ (string_of_int y) ^ ")\n"))]) (offset + 17))                                           
                  ...
                  | Pair(car, cdr) -> (create_const_tbl rest (const_table @ [(Sexpr(Pair(car, cdr)) , (offset , "MAKE_LITERAL_PAIR(const_tbl+"^(string_of_int (find_offset const_table car))^", const_tbl+"^(string_of_int (find_offset const_table car))^")"))]) (offset + 17))
                  | _ -> create_const_list rest const_table offset
)
| [] -> const_table ;; 
...
| Def'(Var'(VarFree(varname)), value) -> (create_fvar_list value (fvar_list @ [varname]))
